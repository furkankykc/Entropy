...
def entropyHesapla(df, tree,f=0,alfa=0.04):
    ...
    for col in range(0, len(df.columns) - 1):
        ...
        for i in df[df.columns[col]].unique():
...
def deng(payToplam,paydaToplam,payOlma,paydaOlma):
    entropy = -(payToplam/paydaToplam)*(payOlma/paydaOlma)*math.log((payOlma/paydaOlma)/(math.pow(2,paydaToplam)-1),2)
...
class entropy:
    ...
    df = None
    def __init__(self,data,func_name,column_list=[]):
...
#a = pd.read_csv('Qualitative_Bankruptcy.data.txt')
...
#entropy('Qualitative_Bankruptcy.data.txt','karci',columns).calc()
#root = Tree()
